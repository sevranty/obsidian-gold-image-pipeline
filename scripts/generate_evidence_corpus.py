...
def main() -> None:
    ...
    for verdict in ("accepted", "repairable", "rejected"):
        ...
        path.write_text(json.dumps({"schema_version": "1.0.0", "verdict": verdict, "cases": [item for item in outputs if item["verdict"] == verdict]}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    checks = sorted(path for path in ROOT.rglob("*.png") if "reports/generated" not in path.as_posix())
    (ROOT / "docs/evidence/ogp8-materialized-checksums.txt").write_text("".join(f"{sha(path)}  {path.relative_to(ROOT).as_posix()}\n" for path in checks), encoding="utf-8")
