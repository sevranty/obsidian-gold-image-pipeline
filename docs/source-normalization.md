...
| Obsidian color | `#000000`; deep obsidian `#1A1A1A` | Background remains `#000000`; object planes may range from near-black to the canonical deep reference `#1A1A1A`. | Separates background identity from readable object tonal modeling. |
| Gold colors | primary `#FFD700`; secondary `#C7A256` | Retain both canonical source references. Lighting may create derived warm-gold variations but not new design colors. | Prevents undocumented palette substitution during public normalization. |
...
| Gold amount | accent only; 15-25% maximum | Use 15-25% as the provisional Foundation target. Below-target use requires semantic justification; above 25% is a critical defect. | Preserves the source range while deferring minimum calibration to visual evidence. |
